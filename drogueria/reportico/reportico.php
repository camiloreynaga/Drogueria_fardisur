 ...
 * @package Reportico
 * @version $Id: reportico.php,v 1.37 2012-05-10 19:53:34 peter Exp $
 */
...
class reportico extends reportico_object
{
	...
	function initialize_panels($mode)
	{
		...
		$this->panels["TOPMENU"]->set_visibility(false);//true
		...
		if ( $mode == "EXECUTE" )
		{
			$this->panels["REPORT"]->set_visibility(true); //true
			//$this->panels["FORM"]->add_panel($this->panels["REPORT"]);
		}
		...
		if ( $mode == "PREPARE" )
		{
			...
			$this->panels["CRITERIA_FORM"]->set_visibility(true);//true
			$this->panels["CRITERIA_EXPAND"]->set_visibility(true); //true
...

		{
			...
			if ( $login_type = $this->login_check($smarty) )
			{
				...
				$this->panels["MENUBUTTON"]->set_visibility(true); //cambiado a false originalmente era true
				$this->panels["LOGIN"]->set_visibility(false);
				$this->panels["SET_ADMIN_PASSWORD"]->set_visibility(false);
				$this->panels["LOGOUT"]->set_visibility(true);
				$this->panels["USERINFO"]->set_visibility(true);
			...
			else
			{
				...
				$this->panels["LOGIN"]->set_visibility(true);
				$this->panels["MENUBUTTON"]->set_visibility(true); //original true
				$this->panels["LOGOUT"]->set_visibility(false);
				$this->panels["USERINFO"]->set_visibility(false);
				$this->panels["RUNMODE"]->set_visibility(false);
	...
	function execute($mode=false, $draw=true)
	{
		...
		switch ($mode) 
		{
			...
			case "PREPARE":
                ...
                if ( $this->xmlinput == "deleteproject.xml" || $this->xmlinput == "configureproject.xml" || $this->xmlinput == "createtutorials.xml" || $this->xmlinput == "createproject" )
                {
    ...
    // Generates a link object against a column
	// -----------------------------------------------------------------------------
	function embed_hyperlink($column_assignee, $label, $url, $open_in_new = true)
	{
            get_query_column($column_assignee, $this->columns )->output_hyperlinks = 
                        array("label" => $label, "url" => $url, "open_in_new" => $open_in_new);
	}
...
class reportico_criteria_column extends reportico_query_column
{
	...
	function format_date_value($in_tag, $in_value, $in_label)
	{
        ...
        try {
            ...
            if ( !$datetime )
            {
                trigger_error("Date value '$in_value' is not in format $in_label<BR>", E_USER_ERROR);
                return "";
